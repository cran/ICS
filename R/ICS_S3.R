...
ICS_covW <- function(x, na.action = na.fail, alpha = 1, cf = 1) {
  ...
  out <- list(location = colMeans(x), scatter = scatter, label = "COVW")
  # add class and return object
  class(out) <- "ICS_scatter"
  out
}
...
ICS <- function(X, S1 = ICS_cov, S2 = ICS_cov4, S1_args = list(),
  ...
  # FIXME: make sure that S2_label is correctly updated (where appropriate)
  if (is.function(S2)) {
  ...
  if (center && (is.null(S1.X$location) || is.null(S2.X$location))) {
  ...
  if (center) {
    ...
    # Andreas: The next part was only needed when 'whiten' and 'center' were
    # both TRUE. The new argument checks above prevent centering when whitening
    # is used, so this part is commented out.
    # if (is.function(S2)) {
    #   S2.X <- get_scatter(X, fun = S2, args = S2_args)
    # } else {
